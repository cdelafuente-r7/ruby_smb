module RubySMB
  module Dcerpc
    module Samr
      ...
      class PsamprServerName < Ndr::NdrPointer
        endian :little

        string16 :referent, length: 2
      end

      class RpcSidIdentifierAuthority < Ndr::NdrFixedByteArray; end
      ...
      class RpcSid < Ndr::NdrStruct
        ...
        rpc_sid_identifier_authority :identifier_authority, length: 6
        ndr_uni_conf_dword_array :sub_authority

        def snapshot
          sid = ['S', self.revision.to_s, self.identifier_authority[-1].to_s]
          self.sub_authority.elements.each { |e| sid << e.to_s }
          sid.join('-')
        end
      end

      class PrpcSid < Ndr::NdrPointer
        endian :little
      ...
      def samr_lookup_domain(server_handle:, name:)
        ...
        samr_lookup_domain_in_sam_server_response.domain_id.referent
      end
